...
import { downloadUrl, objectPathFrom } from '../firebase/storage-url';
 ...
 * same URL — see `storage-url.ts`, which both this and `ArchiveProvider` build from.
 *
...
export class ContentFileProvider {
  ...
  async saveText(itemId: string, text: string): Promise<string> {
    ...
    return downloadUrl(this.config, path, token);
  }
