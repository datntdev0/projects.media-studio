 ...
 *
 * Registered with `app.use()` in main.ts rather than as a Nest middleware
 * class: it takes no dependencies, it has to wrap every route including the
 * docs, and Express 5 no longer accepts the bare `'*'` pattern that
 * `forRoutes()` would need.
 */
